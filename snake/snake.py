...
def move():
    ...
    if (snake.x < 0 or snake.x >= WINDOW_WIDTH or snake.y 
       < 0 or snake.y >= WINDOW_HEIGHT):
        game_over = True
        return
...
def draw():
    ...
    if (game_over):
        canvas.create_text(WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2, 
                           font="Arial 20", text=f"Thua rồi thằng nguu!!! có {score} thôi à! HAHAHA", fill="white")
    else:
        canvas.create_text(30, 20, font="Arial 10", text=f"Score: {score}", 
                           fill="white")
   
    window.after(100, draw)  # 100ms = 1/10 second, 10 frame/second
